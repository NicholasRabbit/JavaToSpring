package com.mybatis.pageHelper.util;
...
public class PageUtil {
	...
	public static String getPageInfo(PageInfo pageInfo,HttpServletRequest resquest) {   //使用HttpServeltRequest要导入servlet-api.jar和jsp-api.jar
		...
		//String path=resquest.getContextPath();  //当做web项目时用此句，这里是实验，没有request对象
		String path="/ssm";
		StringBuffer buffer=new StringBuffer();

		//拼接首页，即第一页
		buffer.append("<a href='"+path+"/emps/1'>首页</a>"+"&nbsp;");  //注意这里拼接的写法，单引号括住的是url地址，而单引号内部用双引号“+xx+”做拼接。
		
		//拼接上一页,首先判断是否有上一页，如果有就拼接上一页的页数int getPrePage();
		if(pageInfo.isHasPreviousPage()) {
			buffer.append("<a href='"+path+"/emps/"+pageInfo.getPrePage()+"'>上一页</a>"+"&nbsp;");
		}else {
			buffer.append("上一页"+"&nbsp;");
		}
		...
		for(int i=0; i<pagesNums.length; i++) {
			if(pagesNums[i] == pageInfo.getPageNum()) {   //条件解释，如果循环页等于pageInfo所代表的当前页面，则设置红色高亮显示，注意拼接的写法。
				...
				buffer.append("<a href='"+path+"/emps/"+pagesNums[i]+"'>"+pagesNums[i]+"</a>"+"&nbsp;");
			}
		...
		if(pageInfo.isHasNextPage()) {
			buffer.append("<a href='"+path+"/emps/"+pageInfo.getNextPage()+"'>下一页</a>"+"&nbsp;");
		}else {
			buffer.append("下一页");
		}
		...
		//拼接尾页
		buffer.append("<a href='"+path+"/emps/"+pageInfo.getPages()+"'>"+"尾页"+"</a>");
